	    ...
	    if (nl_msg->nlmsg_type == RTM_NEWLINK) {
		    ...
		    for(;RTA_OK(rtap, rtl); rtap=RTA_NEXT(rtap, rtl)) {
			    if (rtap->rta_type == IFLA_IFNAME) { /* get interface name */
				    ...
				    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "interface %s \n", iface);
			    }
				    ...
				    int x = *(int *)RTA_DATA(rtap);
				    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "interface linkmode %x \n", x);
			    }
				    ...
				    {
					    case (IF_OPER_DOWN):
						    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "IF_OPER_DOWN");
						    break;
					    case (IF_OPER_UP):
						    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "IF_OPER_UP");
						    break;
					    case (IF_OPER_LOWERLAYERDOWN):
						    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "IF_OPER_LOWERLAYERDOWN");
						    break;
					    case (IF_OPER_TESTING):
						    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "IF_OPER_TESTING");
						    break;
					    case (IF_OPER_DORMANT):
						    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "IF_OPER_DORMANT");
						    break;
					    case (IF_OPER_UNKNOWN):
						    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "IF_OPER_UNKNOWN");
						    break;
					    case (IF_OPER_NOTPRESENT):
						    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "IF_OPER_NOTPRESENT");
						    break;
				    }
				    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "interface new operstate %x \n", x);
			    }
		    }
		    ...
		    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "got a link removal message: family (%d), type (%d), flags(%x) index(%d)\n", linfo->ifi_family, linfo->ifi_type, linfo->ifi_flags, linfo->ifi_index);
//		    done_dhcp = 0;		/* link down */
		    for(;RTA_OK(rtap, rtl); rtap=RTA_NEXT(rtap, rtl)) {
			    if (rtap->rta_type == IFLA_IFNAME) { /* get interface name */
				    strncpy(iface, RTA_DATA(rtap), RTA_PAYLOAD(rtap));
				    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "interface %s \n", iface);
				    ...
				    int x = *(int *)RTA_DATA(rtap);
				    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,  "interface operstate %x \n", x);
			    }
		    }
		    buffer = (char*)malloc (100);
		    memset (buffer, 0, 100);
