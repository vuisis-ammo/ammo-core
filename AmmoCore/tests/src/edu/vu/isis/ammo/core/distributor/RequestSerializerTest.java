...
public class RequestSerializerTest extends AndroidTestCase {
	 ...
	 * ---E ---E ---P
	 * 
	 * --TESTS- These tests focus on the Request Serializer methods. Namely
	 * intricacies of how a specific part behaves with differing content.
	 * =====================================================================
	 */
	...
	public void testNewInstanceArgs() {
		/*
		 * Uri uri = null; Provider p1 = new Provider(uri);
		 * 
		 * Parcel par = utilCreatePayloadParcel(); Payload p2 = new
		 * Payload(par);
		 * 
		 * // Provider.Type.URI, Payload.Type.CV RequestSerializer rs =
		 * RequestSerializer.newInstance(p1,p2); assertNotNull(rs);
		 */
	}
	...
	public void testSerializeFromContentValuesJSON() {
		 ...
		 * assertNotNull(rs);
		 * 
		 * // JSON encoding Encoding encJson =
		 * Encoding.newInstance(Encoding.Type.JSON); byte[] rval =
		 * RequestSerializer.serializeFromContentValues(cv, encJson);
		 */
		assertTrue(true);
	}
	...
	public void testSerializeFromContentValuesTerse() {
		 ...
		 * assertNotNull(rs);
		 * 
		 * // Terse encoding Encoding encTerse =
		 * Encoding.newInstance(Encoding.Type.TERSE); byte[] rval =
		 * RequestSerializer.serializeFromContentValues(cv, encTerse);
		 */
		assertTrue(true);
	}
	 ...
	 * 
	 * Basic use case of serializing from provider. This test
	 * <ol>
	...
	public void testSerializeFromProviderJson() {
		...
		try {
			...
			// if (provider == null) {
			// fail("could not get mock content provider"); }
			...
			final byte[] jsonBlob;
			try {
				jsonBlob = RequestSerializer.serializeFromProvider(cr,
						tupleUri, enc);
			...
			try {
				...
				fail(new StringBuilder()
						.append("could not convert json blob to string")
						.append(jsonBlob).append(" with exception ")
						.append(ex.getLocalizedMessage()).toString());
				return;
			}

			// Examine the JSON string in detail, making sure it contains what
			// we expect
			...
			try {
				...
				assertEquals(json.getInt(AmmoTableSchema.A_FOREIGN_KEY_REF),
						sampleForeignKey);
				assertEquals(
						json.getInt(AmmoTableSchema.AN_EXCLUSIVE_ENUMERATION),
						AmmoTableSchema.AN_EXCLUSIVE_ENUMERATION_HIGH);
				assertEquals(
						json.getInt(AmmoTableSchema.AN_INCLUSIVE_ENUMERATION),
						AmmoTableSchema.AN_INCLUSIVE_ENUMERATION_APPLE);
			} catch (JSONException ex) {
				fail(new StringBuilder()
						.append("Unexpected JSONException :: JSON string =   ")
						.append(jsonString).append(" :: ERROR = ")
						.append(ex.getLocalizedMessage()).toString());
			}
		} finally {
			if (provider != null)
	...
	/**
	 * Serialize to ContentProvider (JSON encoding) Basic use case of
	 * serializing a JSON-encoded message into a provider table.
	 */
	public void testDeserializeToProviderJson() {
		...
		try {
			...
			final Uri tupleIn;
			tupleIn = RequestSerializer.deserializeToProvider(mContext,
					resolver, "dummy", mBaseUri, enc, jsonBytes);

			// We ought to know that the URI... should be row 1, right?
			// assertEquals(ContentUris.withAppendedId(mBaseUri, 1), tupleIn);

			// Now query the provider and examine its contents, checking that
			// they're
			...
			final Cursor cursor = db.query(table, projection, selection,
					selectArgs, groupBy, having, orderBy, limit);

			// The query should have succeeded
			assertNotNull("Query into provider failed", cursor);

			// There should be only one entry
			assertEquals("Unexpected number of rows in cursor", 1,
					cursor.getCount());

			// Row should be accessible with a cursor
			assertTrue("Row not accessible with cursor", (cursor.moveToFirst()));

			// Examine the provider content in detail, making sure it contains
			// what we expect
			// (i.e. the contents of the original JSON)
			assertEquals(sampleForeignKey, cursor.getInt(cursor
					.getColumnIndex(AmmoTableSchema.A_FOREIGN_KEY_REF)));
			assertEquals(
					AmmoTableSchema.AN_EXCLUSIVE_ENUMERATION_HIGH,
					cursor.getInt(cursor
							.getColumnIndex(AmmoTableSchema.AN_EXCLUSIVE_ENUMERATION)));
			assertEquals(
					AmmoTableSchema.AN_INCLUSIVE_ENUMERATION_APPLE,
					cursor.getInt(cursor
							.getColumnIndex(AmmoTableSchema.AN_INCLUSIVE_ENUMERATION)));
		} finally {
			if (provider != null)
				provider.release();
		}
	...
	private Uri utilPopulateTestDbWithCV01(AmmoMockProvider01 provider,
			ContentValues cv) {
		SQLiteDatabase db = provider.getDatabase();
		long rowid = db.insert(Tables.AMMO_TBL,
				AmmoTableSchemaBase.A_FOREIGN_KEY_REF, cv);
	...
	public void testRoundTripJson() {
		...
		this.roundTripTrial(Encoding.newInstance(Encoding.Type.JSON), cv,
				Tables.AMMO_TBL, new SerialChecker() {
					@Override
					public void check(final byte[] bytes) {
						String jsonStr = null;
						try {
	...
	private String compareCVandEncodedBytes(JSONObject jsonObj, ContentValues cv) {
	 ...
	 * @param encoding
	 */
	private void roundTripTrial(Encoding encoding, ContentValues cv,
			String table, SerialChecker checker) {

		((ch.qos.logback.classic.Logger) RequestSerializerTest.logger)
				.setLevel(Level.TRACE);
		((ch.qos.logback.classic.Logger) AmmoMockProviderBase.clogger)
				.setLevel(Level.TRACE);
		((ch.qos.logback.classic.Logger) AmmoMockProviderBase.hlogger)
				.setLevel(Level.TRACE);
		((ch.qos.logback.classic.Logger) RequestSerializer.logger)
				.setLevel(Level.TRACE);
		...
		try {
			...
			final byte[] encodedBytes = encodeTripTrial(provider, resolver,
					encoding, cv, table);
			decodeTripTrial(provider, resolver, encoding, cv, table,
					encodedBytes);
	...
	private byte[] encodeTripTrial(final AmmoMockProvider01 provider,
			final ContentResolver resolver, final Encoding enc,
			final ContentValues cv, final String table) {

		final SQLiteDatabase db = provider.getDatabase();

		long rowid = db
				.insert(table, AmmoTableSchemaBase.A_FOREIGN_KEY_REF, cv);
		final Uri tupleUri = ContentUris.withAppendedId(mBaseUri, rowid);

		// Serialize the provider content into JSON bytes
		final byte[] encodedBytes;
		try {
			encodedBytes = RequestSerializer.serializeFromProvider(resolver,
					tupleUri, enc);
		} catch (NonConformingAmmoContentProvider ex) {
	...
	private void decodeTripTrial(final AmmoMockProvider01 provider,
		...
		final Uri tupleIn = RequestSerializer.deserializeToProvider(mContext,
				resolver, "dummy channel", mBaseUri, enc, encodedBytes);
		...
		// There should be only one entry
		Assert.assertEquals("Unexpected number of rows in cursor", 1,
				cursor.getCount());

		// Row should be accessible with a cursor
		Assert.assertTrue("Row not accessible with cursor",
				(cursor.moveToFirst()));
		...
		for (final Map.Entry<String, Object> entry : cv.valueSet()) {
			...
			if (valueObj instanceof Integer) {
				...
				Assert.fail("unhandled data type"
						+ valueObj.getClass().getCanonicalName());
				return;
			}
		}
	}
