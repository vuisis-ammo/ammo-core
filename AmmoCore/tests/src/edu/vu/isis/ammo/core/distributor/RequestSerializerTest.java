...
public class RequestSerializerTest extends AndroidTestCase {
    ...
    // =========================================================
    /*
     * public void testNewInstanceArgs() { Uri uri = null; Provider p1 = new
     * Provider(uri); Parcel par = utilCreatePayloadParcel(); Payload p2 = new
     * Payload(par); // Provider.Type.URI, Payload.Type.CV RequestSerializer rs
     * = RequestSerializer.newInstance(p1,p2); assertNotNull(rs); }
     */

    /**
     * Serialize from ContentProvider (JSON encoding) : Simple case of known
     * constant values on Table 1 ("Ammo") in schema. This test
     * <ol>
    ...
    /**
     * Serialize from ContentProvider (JSON encoding) : iterated random trials
     * on Table 1 ("Ammo") in schema.
     */
    ...
    /**
     * Serialize from ContentProvider (JSON encoding) : Simple case of known
     * constant values on Table 2 ("Quick") in schema. This test
     * <ol>
     * <li>constructs a mock content provider,
     * <li>loads some data into the content provider,(imitating the application)
     * <li>serializes that data into a json string
    ...
    /**
     * Serialize from ContentProvider (JSON encoding) Simple case of known
     * constant values on Table 3 ("Start") in schema. This test
     * <ol>
     * <li>constructs a mock content provider,
     * <li>loads some data into the content provider,(imitating the application)
     * <li>serializes that data into a json string
    ...
    /**
     * Serialize from ContentProvider (JSON encoding) iterated random trials on
     * Table 3 ("Start") in schema.
     */
    ...
    /**
     * Serialize to ContentProvider (JSON encoding) for Table 1 ("Ammo"). Basic
     * use case of serializing a JSON-encoded message into a provider table.
     */
    public void testDeserializeToProviderJson_table1_basic()
    {
        ...
        try {
            ...
            final SchemaTable1Data d = new SchemaTable1Data();
            ...
            RequestSerializer.deserializeToProvider(aqh,
                    mContext,
                    cr,
                    "dummy",
                    d.getBaseUri(),
    ...
    /**
     * Serialize to ContentProvider (JSON encoding) for Table 2 ("Quick"). Basic
     * use case of serializing a JSON-encoded message into a provider table.
     */
    public void testDeserializeToProviderJson_table2_basic()
    {
        ...
        try {
            ...
            final SchemaTable2Data d = new SchemaTable2Data();
    ...
    /**
     * Serialize to ContentProvider (JSON encoding) for Table 3 ("Start"). Basic
     * use case of serializing a JSON-encoded message into a provider table.
     */
    public void testDeserializeToProviderJson_table3_basic()
    {
        ...
        try {
            ...
            // Object with "Table 2" knowledge
            final SchemaTable3Data d = new SchemaTable3Data();
    ...
    /**
     * Serialize from ContentProvider (JSON encoding) : Simple case of known
     * constant values on Table 3 ("Start") in schema. WITH BLOB DATA
     */
    ...
    /**
     * Serialize from ContentProvider (JSON encoding) : Repeatedly serialize
     * random values on Table 3 ("Start") in schema. WITH BLOB DATA
     */
    public void testSerializeFromProviderJson_withBlob_random_smallBlob() {
        ...
        try {
            ...
            /*
             * catch (OutOfMemoryError e) { // This won't happen most of the
             * time, but catch if it does. // Don't call it a test failure, just
             * log and return. Log.e(TAG, "*** out of memory error ***");
     ...
     * TODO } public void
     * testSerializeFromProviderJson_withBlob_random_largeBlob() { // TODO }
     * public void testSerializeFromProviderJson_withBlob_errors_smallBlob() {
     * // TODO } public void
     * testSerializeFromProviderJson_withBlob_errors_largeBlob() { // TODO }
     * public void testDeserializeToProviderJson_withBlob_basic_smallBlob() { //
     * TODO } public void
     * testDeserializeToProviderJson_withBlob_basic_largeBlob() { // TODO }
     * public void testDeserializeToProviderJson_withBlob_random_smallBlob() {
     * // TODO } public void
     * testDeserializeToProviderJson_withBlob_random_largeBlob() { // TODO }
     * public void testSerializeFromProviderJson_basic_withFile() { // TODO }
     */

    /**
     * Serialize from ContentProvider (Terse encoding) : Simple case of known
     * constant values on Table 1 ("Ammo") in schema. This test
     * <ol>
     * <li>constructs a mock content provider,
     * <li>loads some data into the content provider,(imitating the application)
