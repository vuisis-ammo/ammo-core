	...
	public void setWifiStatus() {
	    ...
	    Thread wifiThread = new Thread() {
		    public void run() {
				...
				WifiManager manager = (WifiManager)WiredNetlink.this.context.getSystemService(Context.WIFI_SERVICE);
				WifiInfo info = manager.getConnectionInfo();
				logger.debug( "WifiInfo: " +  info.toString() );
				boolean wifiConn = (info != null && info.getSupplicantState() == SupplicantState.COMPLETED);
				Editor editor = PreferenceManager.getDefaultSharedPreferences(WiredNetlink.this.context).edit();
