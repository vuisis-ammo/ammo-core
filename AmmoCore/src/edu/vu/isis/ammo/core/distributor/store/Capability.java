...
public class Capability {
	...
	static public void onCreate(SQLiteDatabase db) {
		...
		try {	
			...
			.append(table.q())
			.append(" ( ")
			.append(TableHelper.ddl(CapabilityField.values())).append(')')
			.append(';');
	...
	public static class CapabilityWorker {
		...
		public long upsert() {
			...
			synchronized(this.parent) {	
				...
				try {
					...
					long row = this.db.insert(Tables.CAPABILITY.n, CapabilityField._ID.n(), cv);
		...
		public int delete(String tupleId) {
			...
			try {
				final int count = this.db.delete(Tables.CAPABILITY.n, whereClause, whereArgs);
