 ...
 * 
 */
public class ChannelAdapter extends ArrayAdapter<ModelChannel> implements
		OnTouchListener, OnNameChangeListener {
	...
	public ChannelAdapter(AmmoCore parent, List<ModelChannel> model) {
		super(parent, android.R.layout.simple_list_item_1, model);
		this.parent = parent;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		View row = channel
				.getView(convertView, this.parent.getLayoutInflater());
	...
	public boolean onTouch(View view, MotionEvent event) {
		Toast.makeText(this.getContext(), "Event: " + event.getAction(),
	...
	private boolean onStatusChange(View item, ModelChannel channel) {
		...
		String channelType = (String) ((TextView) row
				.findViewById(R.id.channel_type)).getText();
		if (channelType.equals(Gateway.KEY)) {
			text_one = (TextView) row
					.findViewById(R.id.gateway_status_text_one);
			text_two = (TextView) row
					.findViewById(R.id.gateway_status_text_two);

			text_send = (TextView) row.findViewById(R.id.gateway_send_stats);
			text_receive = (TextView) row
					.findViewById(R.id.gateway_receive_stats);
			...
			text_receive = (TextView) row
					.findViewById(R.id.multicast_receive_stats);
		} else if (channelType.equals(ReliableMulticast.KEY)) {
			text_one = (TextView) row
					.findViewById(R.id.reliable_multicast_status_one);
			text_two = (TextView) row
					.findViewById(R.id.reliable_multicast_status_two);

			text_send = (TextView) row
					.findViewById(R.id.reliable_multicast_send_stats);
			text_receive = (TextView) row
					.findViewById(R.id.reliable_multicast_receive_stats);
		...
		if (parent.netlinkAdvancedView) {
			if (channel.getClass() == Serial.class) {
				...
				countsString.append("S:").append(ch.getMessagesSent())
						.append(" ");
				...
				errorString.append("@:").append(ch.getCorruptMessages())
						.append(" ");
				errorString.append(ch.getReceiverSubstate());
				errorString.append(":").append(ch.getBytesSinceMagic())
						.append(" ");
				...
				case INetChannel.CONNECTED:
					...
					case INetChannel.WAIT_RECONNECT:
						...
						break;
					default:
						logger.error("missing sender status handling {}",
								status[1]);
					...
					case INetChannel.RESTART:
						...
						break;
					default:
						logger.error("missing receiver status handling {}",
								status[2]);
						setColor(icon, text, R.color.status_unknown);
						text.setText(R.string.status_unknown);
					}
				...
				if (text_send != null)
					text_send
							.setText(channel.getNetChannel().getSendBitStats());
				if (text_receive != null)
					text_receive.setText(channel.getNetChannel()
							.getReceiveBitStats());
			}
		}
		...
		// User Mode
			...
			/*
			 * Pending | Transmitting Sending | Transmitting Taking |
			 * Transmitting Start | Transmitting Started | Transmitting
			 * Exception | Error: Restart Omma Waiting | Error: Restart Omma
			 * Interrupted | Error: Restart Omma Connecting | Connecting Linking
			 * | Connecting Linked | Connecting Disconnected | Disconnected
			 * Shutdown | Disconnected Stale | Checking Status
			 */
			switch (status[0]) {
	...
	private void setColor(ToggleButton icon, TextView text, int resColor) {
		...
		if (icon != null)
			icon.setTextColor(R.color.togglebutton_default);
		if (text != null)
