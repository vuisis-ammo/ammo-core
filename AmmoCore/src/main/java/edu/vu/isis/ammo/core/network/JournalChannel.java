...
public class JournalChannel extends NetChannel {
    ...
    private BlockingQueue<AmmoGatewayMessage> sendQueue = new LinkedBlockingQueue<AmmoGatewayMessage>(
            20);

    private boolean isStale;
    private boolean isEnabled;

    public static final File journalDir = new File(Environment.getExternalStorageDirectory(),
            "ammo_core");
    ...
    public boolean isJournaling() {
        return journalingSwitch;
    }

    public void setJournalSwitch(boolean value) {
        journalingSwitch = value;
    }

    public boolean getJournalSwitch() {
        return journalingSwitch;
    }

    public boolean toggleJournalSwitch() {
        return journalingSwitch = journalingSwitch ? false : true;
    }

    private JournalChannel(String name, NetworkManager driver) {
        super(name);
    ...
    public static JournalChannel getInstance(String name, NetworkManager driver) {
    ...
    /**
     * The journal used when direct communication with the ammo android gateway
     * plugin is not immediately available. The journal is a file containing the
     * PushRequests (not RetrievalRequest's or *Response's).
     */
    ...
    /**
     * We don't need to reconnect unless. 1) the connection has been lost 2) the
     ...
     * Close the socket.
     * 
     * @return whether the socket was closed. false may simply indicate that the
    ...
    /**
     * A thread for receiving incoming messages on the tcp socket. The main
     * method is run().
     */
    public class SenderThread extends Thread {
        ...
        /**
         * Initiate a connection to the server and then wait for a response. All
         * responses are of the form: size : int32 payload_checksum : int32
         * bytes[] : <size> This is done via a simple state machine. If the
         * payload_checksum doesn't match the connection is dropped and
         * restarted. Once the message has been read it is passed off to...
         */
        @Override
        public void run() {
            logger.info("::run");
            ...
            try {
                ...
                ByteBuffer buf = ByteBuffer.allocate(Integer.SIZE / Byte.SIZE + Integer.SIZE
                        / Byte.SIZE);
                    ...
                    synchronized (JournalChannel.isConnected) {
                        while (!JournalChannel.isConnected) {
                            try {
                                ...
                                return; // looks like the thread is being shut
                                        // down.
                            }
                        }
                    }
                    AmmoGatewayMessage msg = queue.take();
                    dos.write(msg.serialize(endian, AmmoGatewayMessage.VERSION_1_FULL, (byte) 0)
                            .array());
                }
