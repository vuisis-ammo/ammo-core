...
public class DistributorThread extends Thread {
	...
	private static final Logger resLogger = LoggerFactory
			.getLogger("test.queue.response");
	private static final Logger reqLogger = LoggerFactory
			.getLogger("test.queue.request");
	private static final boolean RUN_TRACE = false;

	private static final Marker MARK_POSTAL = MarkerFactory.getMarker("postal");
	private static final Marker MARK_RETRIEVAL = MarkerFactory
			.getMarker("retrieval");
	private static final Marker MARK_SUBSCRIBE = MarkerFactory
			.getMarker("subscribe");

	private static final String ACTION_BASE = "edu.vu.isis.ammo.";
	public static final String ACTION_MSG_SENT = ACTION_BASE
			+ "ACTION_MESSAGE_SENT";
	public static final String ACTION_MSG_RCVD = ACTION_BASE
	...
	public DistributorThread(final Context context, final NetworkManager parent) {
		super(new StringBuilder("Distribute-").append(
				DistributorThread.gThreadOrdinal.getAndIncrement()).toString());
	...
	private class NotifyMsgNumber implements Runnable {
		...
		private void updateNotification() {
			...
			String contentText = "Sent " + total_sent + " Received "
					+ total_recv;
			...
			if (channelStatus.get() == false) {
				...
				notifyIcon("Omma Channel Down", "Data Channel", "Offline",
						icon1);
				return;
			}
	 ...
	 * @param change
	 */
	public void onChannelChange(final Context context,
			final String channelName, final ChannelChange change) {
	...
	private void notifyIcon(String tickerTxt, String contentTitle,
			String contentText, int icon) {
	...
	private class ChannelAck {
		...
		public ChannelAck(Relations type, long id, UUID uuid, String topic,
				String auid, Notice notice, String channel, DisposalState status) {
			this.type = type;
			this.id = id;
			this.uuid = uuid;
			this.topic = topic;
		...
		public String toString() {
			return new StringBuilder().append(" type ").append(type)
					.append(" id ").append(id).append(" uuid ").append(uuid)
					.append(" topic ").append(topic).append(" aid ")
					.append(auid).append(" channel ").append(channel)
	 ...
	 * send the message. The acknowledged message count is updated and a
	...
	private void doChannelAck(final Context context, final ChannelAck ack) {
		...
		switch (ack.type) {
		case POSTAL:
			numUpdated = this.store.updatePostalByKey(ack.id, ack.channel,
					ack.status);
			break;
		case RETRIEVAL:
			numUpdated = this.store.updateRetrievalByKey(ack.id, ack.channel,
					ack.status);
			break;
		case SUBSCRIBE:
			numUpdated = this.store.updateSubscribeByKey(ack.id, ack.channel,
					ack.status);
			break;
		default:
		...
		if (ack.notice != null) {
			...
			if (via.isActive()) {
				...
				final Notice.IntentBuilder noteBuilder = Notice
						.getIntentBuilder(ack.notice).topic(ack.topic)
						.auid(ack.auid).channel(ack.channel);
				...
				PLogger.API_INTENT.debug("ack note=[{}] intent=[{}]", note,
						noticed);
	...
	public String distributeRequest(AmmoRequest request) {
		try {
			logger.info("From AIDL into AMMO type:{} uuid:{}", request.topic,
					request.uuid);

			PLogger.QUEUE_REQ_ENTER.trace(
					"\"action\":\"offer\" \"request\":\"{}\"", request);
	...
	public boolean distributeResponse(AmmoGatewayMessage agm) {
		PLogger.QUEUE_RESP_ENTER.trace(
				"\"action\":\"offer\" \"response\":\"{}\"", agm);
	...
	public void run() {
		...
		final AtomicLong sanitationSchedule = new AtomicLong(
				System.currentTimeMillis() + (1 * 60 * 60 * 1000));
		// initial sanitation work happens after 1 hour (in milliseconds)

		if (networkManager.isConnected()) {

			for (final Map.Entry<String, ChannelStatus> entry : channelStatus
					.entrySet()) {
		...
		try {
			while (true) {
				...
				while (this.isReady()) {
					...
					if (!this.channelAck.isEmpty()) {
						...
						try {
							...
							PLogger.QUEUE_ACK_EXIT.trace(PLogger.QUEUE_FORMAT,
									new Object[] { this.channelAck.size(),
											ack.uuid, 0, ack });

							this.doChannelAck(this.context, ack);
						} catch (ClassCastException ex) {
							logger.error(
									"channel ack queue contains illegal item of class",
									ex);
						}
					}
					...
					if (!this.responseQueue.isEmpty()) {
						try {
							final AmmoGatewayMessage agm = this.responseQueue
									.take();
							PLogger.QUEUE_RESP_EXIT
									.trace(PLogger.QUEUE_FORMAT,
											new Object[] {
							...
							logger.info(
									...
									new Object[] { agm.payload_checksum,
											agm.buildTime,
											this.responseQueue.size() });
							...
							logger.error(
									"response queue contains illegal item of class",
									ex);
						}
					}
					...
					if (!this.requestQueue.isEmpty()) {
						try {
							...
							logger.info(
									"processing request uuid {}, remaining {}",
									ar.uuid, this.requestQueue.size());
							reqLogger.info(
									PLogger.TEST_QUEUE_FORMAT,
									new Object[] { System.currentTimeMillis(),
											"request_queue",
											this.requestQueue.size() });
							PLogger.QUEUE_REQ_EXIT.trace(PLogger.QUEUE_FORMAT,
									new Object[] { this.requestQueue.size(),
											ar.uuid, "n/a", ar });

							this.doRequest(networkManager, ar);
						} catch (ClassCastException ex) {
							logger.error(
									"request queue contains illegal item of class",
									ex);
						}
					}
				}
	...
	private boolean doResponse(Context context, AmmoGatewayMessage agm) {
		...
		case DATA_MESSAGE:
		case TERSE_MESSAGE:
			final boolean subscribeResult = receiveSubscribeResponse(context,
					mw, agm.channel, agm.priority);
			...
			if (mw.hasDataMessage()) {
				...
				if (dm.hasOriginDevice()) {
					Presence.getWorker().device(dm.getOriginDevice())
							.operator(dm.getUserId()).upsert();
				}
			}
			break;
		...
		case PULL_RESPONSE:
			final boolean retrieveResult = receiveRetrievalResponse(context,
					mw, agm.channel, agm.priority);
		...
		case SUBSCRIBE_MESSAGE:
			if (mw.hasSubscribeMessage()) {
				final AmmoMessages.SubscribeMessage sm = mw
						.getSubscribeMessage();
				final FullTopic fulltopic = FullTopic
						.fromType(sm.getMimeType());

				if (sm.hasOriginDevice()) {
					final String deviceId = sm.getOriginDevice();
					final String operator = sm.getOriginUser();

					Capability.getWorker().origin(deviceId).operator(operator)
							.topic(fulltopic.topic)
							.subtopic(fulltopic.subtopic).upsert();

					Presence.getWorker().device(deviceId).operator(operator)
							.upsert();
				}
			}
			break;
	 ...
	 * @return
	 */
	private boolean receiveAuthenticateResponse(Context context,
			AmmoMessages.MessageWrapper mw) {
	...
	private void doPostalRequest(final NetworkManager that, final AmmoRequest ar) {
		...
		try {
			...
			final DistributorPolicy.Topic policy = that.policy().matchPostal(
					topic);
			final String channel = (ar.channelFilter == null) ? null
					: ar.channelFilter.cv();
			...
			values.put(PostalTableSchema.PRIORITY.cv(),
					policy.routing.getPriority(ar.priority));
			values.put(PostalTableSchema.EXPIRATION.cv(),
					policy.routing.getExpiration(ar.expire.cv()));

			values.put(PostalTableSchema.CREATED.cv(),
			...
			if (!that.isConnected()) {
				values.put(PostalTableSchema.DISPOSITION.cv(),
						DisposalTotalState.NEW.cv());
				long key = this.store.upsertPostal(values,
						policy.makeRouteMap(channel));
				logger.debug("no channel connected, added postal [{}] [{}]",
						key, values);
				return;
			}

			final RequestSerializer serializer = RequestSerializer.newInstance(
					ar.provider, ar.payload);
			serializer.setSerializer(new RequestSerializer.OnSerialize() {
				...
				public byte[] run(Encoding encode) {
					...
					if (serializer_.payload.whatContent() != Payload.Type.NONE) {
						final byte[] result = RequestSerializer
								.serializeFromContentValues(
										serializer_.payload.getCV(), encode);
						...
						try {
							final byte[] result = RequestSerializer
									.serializeFromProvider(that_.getContext()
											.getContentResolver(),
											serializer_.provider.asUri(),
											encode);

							if (result == null) {
								logger.error(
										"Null result from serialize {} {} ",
							...
							parent.store()
									.deletePostal(
											new StringBuilder()
													.append(PostalTableSchema.PROVIDER
															.q())
													.append("=?")
													.append(" AND ")
													.append(PostalTableSchema.TOPIC
															.q()).append("=?")
													.toString(),
											new String[] {
													e.missingTupleUri.getPath(),
													topic });
							return null;
			...
			synchronized (this.store) {
				final long id = this.store.upsertPostal(values,
						policy.makeRouteMap(channel));

				final Dispersal dispatchResult = this.dispatchPostalRequest(
						that, ar.notice, uuid, topic, dispersal, serializer,
						new INetworkService.OnSendMessageHandler() {
							...
							@Override
							public boolean ack(String channel,
									DisposalState status) {
								final ChannelAck chack = new ChannelAck(
										Relations.POSTAL, id_, uuid_, topic_,
										auid_, notice_, channel, status);
	...
	private void cancelPostalRequest(final NetworkManager that,
			final AmmoRequest ar) {
		...
		try {
			...
			this.store().deletePostal(
					new StringBuilder().append(PostalTableSchema.PROVIDER.q())
							.append("=?").append(" AND ")
							.append(PostalTableSchema.TOPIC.q()).append("=?")
							.toString(), new String[] { provider, topic });
		} finally {
	...
	private void doPostalCache(final NetworkManager that) {
		...
		for (boolean moreItems = pending.moveToFirst(); moreItems; moreItems = pending
				.moveToNext()) {
			final int id = pending.getInt(pending
					.getColumnIndex(PostalTableSchema._ID.n));
			final String auid = pending.getString(pending
					.getColumnIndex(PostalTableSchema.AUID.n));
			...
			final int noticeIx = pending
					.getColumnIndex(PostalTableSchema.NOTICE.n);
			final Notice notice;
			...
			final RequestSerializer serializer = RequestSerializer.newInstance(
					provider, payload);
			final String orderingMethodId = pending.getString(pending
					.getColumnIndex(PostalTableSchema.ORDER.n));
			@SuppressWarnings("unused")
			final Order orderMethod = new Order(orderingMethodId);

			final SerializeMode serialType;
			int dataColumnIndex = pending
					.getColumnIndex(PostalTableSchema.DATA.n);
				...
				public byte[] run(Encoding encode) {
					...
					default:
						try {
							...
							logger.error(
									"no tuple for postal request serializer [{}]",
									serializer_);
							parent.store()
									.deletePostal(
											new StringBuilder()
			...
			final Dispersal dispersal = policy.makeRouteMap(channelFilter);
			{
				...
				for (boolean moreChannels = channelCursor.moveToFirst(); moreChannels; moreChannels = channelCursor
						.moveToNext()) {
					final String channel = channelCursor
							.getString(channelCursor
									.getColumnIndex(DisposalTableSchema.CHANNEL.n));
					final short channelState = channelCursor
							.getShort(channelCursor
									.getColumnIndex(DisposalTableSchema.STATE.n));
					dispersal.put(channel,
							DisposalState.getInstanceById(channelState));
				}
			...
			try {
				...
				synchronized (this.store) {
					...
					final Dispersal dispatchResult = this
							.dispatchPostalRequest(that, notice, uuid, topic,
									...
									new INetworkService.OnSendMessageHandler() {
										...
										public boolean ack(String channel,
												DisposalState status) {
											final ChannelAck chack = new ChannelAck(
													Relations.POSTAL, id_,
													uuid_, topic_, auid_,
													notice_, channel, status);
											return parent
													.announceChannelAck(chack);
										}
									});
					this.store.updatePostalByKey(id, null, dispatchResult);
				}
			...
			public AmmoGatewayMessage run(Encoding encode, byte[] serialized) {
				...
				if (encode.getType() != Encoding.Type.TERSE) {
					final AmmoMessages.DataMessage.Builder pushReq = AmmoMessages.DataMessage
							.newBuilder().setUri(uuid.toString())
							.setMimeType(msgType)
					...
					if (notice_ != null) {
						final AcknowledgementThresholds.Builder noticeBuilder = AcknowledgementThresholds
								.newBuilder()
								.setDeviceDelivered(
										notice_.atDeviceDelivered.getVia()
												.isActive())
								.setAndroidPluginReceived(
										notice_.atGatewayDelivered.getVia()
												.isActive())
								.setPluginDelivered(
										notice_.atPluginDelivered.getVia()
												.isActive());
				...
				logger.debug(
						"Finished wrap build @ timeTaken {} ms, serialized-size={} \n",
						System.currentTimeMillis() - now, serialized.length);
				final AmmoGatewayMessage.Builder agmb = AmmoGatewayMessage
						.newBuilder(mw, handler);
				agmb.needAck(notice_).uuid(uuid);
				return agmb.build();
			}
		});
	 ...
	 * @return
	 */
	private boolean receivePostalResponse(Context context,
			AmmoMessages.MessageWrapper mw, NetChannel channel) {
		...
		final Cursor postalReq = this.store.queryPostal(
				null,
				new StringBuilder()
						.append(DistributorDataStore.PostalTableSchema.UUID
								.cv()).append("=?").toString(),
				new String[] { pushResp.getUri() }, null);
		...
		if (pushResp.hasThreshold()) {
			...
			byte[] noticeBytes = postalReq
					.getBlob(postalReq
							.getColumnIndex(DistributorDataStore.PostalTableSchema.NOTICE
									.cv()));
			logger.debug("notice bytes {}", noticeBytes);
			final Notice notice = Notice.unpickle(noticeBytes);

			final String topic = postalReq
					.getString(postalReq
							.getColumnIndex(DistributorDataStore.PostalTableSchema.TOPIC
									.cv()));
			final String auid = postalReq.getString(postalReq
					.getColumnIndex(DistributorDataStore.PostalTableSchema.AUID
							.cv()));
			...
			final Notice.IntentBuilder noteBuilder = Notice
					.getIntentBuilder(notice).topic(topic).auid(auid)
			...
			PLogger.API_INTENT
					.debug("ack note=[{}] intent=[{}]", note, noticed);
			if (PLogger.API_INTENT.isTraceEnabled()) {
				PLogger.API_INTENT.trace("extras=[{}]",
						PLogger.expandBundle(noticed.getExtras(), '\n'));
			}
	...
	static private void sendIntent(int aggregate, Intent noticed,
			Context context) {
		if (0 < (aggregate & Via.Type.ACTIVITY.v)) {
			noticed.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			context.startActivity(noticed);
	...
	private void doRetrievalRequest(NetworkManager that, AmmoRequest ar) {
		logger.trace("process request RETRIEVAL {} {}", ar.topic.toString(),
		...
		try {
			...
			final Integer limit = (ar.limit == null) ? null : ar.limit
					.asInteger();
			final DistributorPolicy.Topic policy = that.policy()
					.matchRetrieval(topic);
			...
			values.put(RetrievalTableSchema.PRIORITY.cv(),
					policy.routing.getPriority(ar.priority));
			...
			values.put(RetrievalTableSchema.CREATED.cv(),
					System.currentTimeMillis());
			...
			if (!that.isConnected()) {
				...
				logger.debug("no channel available, added retrieval [{}] [{}]",
						key, values);
				return;
			}
			...
			synchronized (this.store) {
				final long id = this.store.upsertRetrieval(values,
						policy.makeRouteMap(null));

				final Dispersal dispatchResult = this.dispatchRetrievalRequest(
						that, uuid, topic, select, limit, dispersal,
						new INetworkService.OnSendMessageHandler() {
							final DistributorThread parent = DistributorThread.this;
							final long id_ = id;
							final UUID uuid_ = uuid;
							...
							public boolean ack(String channel,
									DisposalState status) {
								final ChannelAck chack = new ChannelAck(
										Relations.RETRIEVAL, id_, uuid_,
										topic_, auid_, notice_, channel, status);
								return parent.announceChannelAck(chack);
							}
						});
	...
	private void cancelRetrievalRequest(final NetworkManager that,
			final AmmoRequest ar) {
		...
		try {
			...
			final String selection = new StringBuilder()
					...
					.append(" AND ").append(RetrievalTableSchema.TOPIC.q())
					.append("=?").toString();
	...
	private void doRetrievalCache(NetworkManager that) {
		...
		for (boolean areMoreItems = pending.moveToFirst(); areMoreItems; areMoreItems = pending
			...
			final int id = pending.getInt(pending
					.getColumnIndex(RetrievalTableSchema._ID.n));
			final String topic = pending.getString(pending
			...
			final String auid = pending.getString(pending
					.getColumnIndex(RetrievalTableSchema.AUID.cv()));
			final String selection = pending.getString(pending
					.getColumnIndex(RetrievalTableSchema.SELECTION.n));

			final int columnIx = pending
					.getColumnIndex(RetrievalTableSchema.LIMIT.n);
			final Integer limit = pending.isNull(columnIx) ? null : pending
					.getInt(columnIx);
			...
			try {
				...
				synchronized (this.store) {
					...
					values.put(RetrievalTableSchema.DISPOSITION.cv(),
							DisposalTotalState.DISTRIBUTE.cv());
					@SuppressWarnings("unused")
					final long numUpdated = this.store.updateRetrievalByKey(id,
							values, null);

					final Dispersal dispatchResult = this
							.dispatchRetrievalRequest(that, uuid, topic,
									selection, limit, dispersal,
									new INetworkService.OnSendMessageHandler() {
										final DistributorThread parent = DistributorThread.this;
										final String auid_ = auid;
										final UUID uuid_ = uuid;
	...
	private Dispersal dispatchRetrievalRequest(final NetworkManager that,
			final UUID retrievalId, final String topic, final String selection,
			final Integer limit, final Dispersal dispersal,
			final INetworkService.OnSendMessageHandler handler) {
		...
		final AmmoMessages.PullRequest.Builder retrieveReq = AmmoMessages.PullRequest
				.newBuilder().setRequestUid(retrievalId.toString())
				.setMimeType(topic);
		...
		// expiration
		try {
			final RequestSerializer serializer = RequestSerializer
					.newInstance();
			serializer.setAction(new RequestSerializer.OnReady() {
	 ...
	 * @return
	 */
	private boolean receiveRetrievalResponse(Context context,
			AmmoMessages.MessageWrapper mw, NetChannel channel, int priority) {
		if (mw == null)
			return false;
		...
		final Cursor cursor = this.store.queryRetrievalByKey(
				new String[] { RetrievalTableSchema.PROVIDER.n }, uuidString,
				topic, null);
		if (cursor.getCount() < 1) {
			logger.error(
					"received a message for which there is no retrieval {} {}",
					topic, uuidString);
			cursor.close();
			return false;
		}
		...
		final Encoding encoding = Encoding
				.getInstanceByName(resp.getEncoding());
		final boolean queued = this.deserialThread.toProvider(priority,
				context, channel.name, provider, encoding, resp.getData()
						.toByteArray());
	 ...
	 * @param st
	 */
	private void doSubscribeRequest(final NetworkManager that,
			final AmmoRequest ar, int st) {
		...
		try {
			...
			values.put(SubscribeTableSchema.SELECTION.cv(),
					ar.select.toString());

			values.put(SubscribeTableSchema.PRIORITY.cv(),
					policy.routing.getPriority(ar.priority));
			values.put(SubscribeTableSchema.EXPIRATION.cv(),
					policy.routing.getExpiration(ar.expire.cv()));
			values.put(SubscribeTableSchema.CREATED.cv(),
					System.currentTimeMillis());
			...
			if (!that.isConnected()) {
				...
				logger.debug("no channel available, added subscribe [{}] [{}]",
						key, values);
				return;
			}
			...
			synchronized (this.store) {
				...
				final Dispersal dispatchResult = this.dispatchSubscribeRequest(
						that, topic, ar.select.toString(), dispersal,
						new INetworkService.OnSendMessageHandler() {
							final DistributorThread parent = DistributorThread.this;
							final long id_ = id;
							final UUID uuid_ = uuid;
	...
	private void cancelSubscribeRequest(final NetworkManager that,
			final AmmoRequest ar) {
		...
		try {
			...
			final String selection = new StringBuilder()
					...
					.append(" AND ").append(SubscribeTableSchema.TOPIC.q())
					.append("=?").toString();
			final String[] selectionArgs = new String[] { provider, topic };
	...
	private void doSubscribeCache(NetworkManager that) {
		...
		for (boolean areMoreItems = pending.moveToFirst(); areMoreItems; areMoreItems = pending
			...
			// serialize it, then pass it off to the NPS.
			final int id = pending.getInt(pending
					.getColumnIndex(SubscribeTableSchema._ID.n));
			final String uuidString = pending.getString(pending
					.getColumnIndex(PostalTableSchema.UUID.n));
			final UUID uuid = UUID.fromString(uuidString);
			...
			try {
				...
				synchronized (this.store) {
					...
					values.put(SubscribeTableSchema.DISPOSITION.cv(),
							DisposalTotalState.DISTRIBUTE.cv());
					@SuppressWarnings("unused")
					long numUpdated = this.store.updateSubscribeByKey(id,
							values, null);

					final Dispersal dispatchResult = this
							.dispatchSubscribeRequest(that, topic, selection,
									dispersal,
									new INetworkService.OnSendMessageHandler() {
										final DistributorThread parent = DistributorThread.this;
										final int id_ = id;
	...
	private Dispersal dispatchSubscribeRequest(final NetworkManager that,
			final String topic, final String selection,
		...
		subscribeReq.setOriginDevice(this.networkManager.getDeviceId())
	 ...
	 */
	private boolean receiveSubscribeResponse(Context context,
			AmmoMessages.MessageWrapper mw, NetChannel channel, int priority) {
		if (mw == null) {
		...
		if (originUser.equals(selfOperator)) {
			logger.error("received own user message [{}:{}]", originUser,
					selfOperator);
		}
		...
		final Cursor cursor = this.store.querySubscribeByKey(
				new String[] { SubscribeTableSchema.PROVIDER.n }, topic, null);
		if (cursor.getCount() < 1) {
			logger.warn(
					"received a message for which there is no subscription {}",
					topic);
			cursor.close();
			return false;
		}
		...
		if (at != null && at.getDeviceDelivered()) {
			final AcknowledgementThresholds bt = AcknowledgementThresholds
					.newBuilder().setDeviceDelivered(true).build();

			final AmmoMessages.PushAcknowledgement.Builder pushAck = AmmoMessages.PushAcknowledgement
					.newBuilder().setUri(originUid).setThreshold(bt)
			...
			final AmmoMessages.MessageWrapper.Builder mwb = AmmoMessages.MessageWrapper
					.newBuilder()
					.setType(
							AmmoMessages.MessageWrapper.MessageType.PUSH_ACKNOWLEDGEMENT)
					.setPushAcknowledgement(pushAck);

			final AmmoGatewayMessage.Builder oagmb = AmmoGatewayMessage
					.newBuilder(mwb,
							new INetworkService.OnSendMessageHandler() {
								// final
								// AmmoMessages.PushAcknowledgement.Builder ack_
								// =
								// pushAck;
								@Override
								public boolean ack(String channel,
		...
		this.deserialThread.toProvider(priority, context, channel.name,
				provider, encoding, data.toByteArray());

		logger.info("Ammo received message on topic: {} for provider: {}",
				mime, uriString);
