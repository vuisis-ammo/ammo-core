...
/**
 * <p>
 * The NetworkManager is responsible for prioritizing and serializing requests
 * for data communications between distributed application databases. The
 * NetworkManager issues calls to the NetworkManager for updates and then writes
 * the results to the correct content provider using the deserialization
 * mechanism defined by each content provider.
 * <p>
 * Any activity or application wishing to send data via the NetworkManager
 * should use one of the AmmoRequest API methods for communication between said
...
public enum NetworkManager implements INetworkService,
    ...
    private boolean isSSLSuppressed = INetPrefKeys.DEFAULT_SSL_ENABLED;
    ...
    static final private AtomicBoolean isStartCommandSuppressed = new AtomicBoolean(
            false);
    ...
    final private AtomicReference<DistributorPolicy> policy = new AtomicReference<DistributorPolicy>();
    ...
    private void onCreate() {
        ...
        /**
         * TODO none of these INIT actually do anything right now
         */
        ...
        /**
         * TODO this registered Channels list is not being used either
         */
        ...
        this.mReceiverRegistrar = new IRegisterReceiver() {
            ...
            public Intent registerReceiver(final BroadcastReceiver aReceiver,
                ...
                return NetworkManager.this.context.registerReceiver(aReceiver,
                        aFilter);
            }
        ...
        mReceiverRegistrar
                .registerReceiver(mReadyResourceReceiver, readyFilter);
        ...
        mReceiverRegistrar
                .registerReceiver(mReadyResourceReceiver, mediaFilter);
        ...
        this.localSettings = PreferenceManager
                .getDefaultSharedPreferences(this.context);
        this.localSettings
                .registerOnSharedPreferenceChangeListener(this.ammoPreferenceChangeListener);

        try {
            this.serialChannel = new SerialChannel(ChannelFilter.SERIAL, this,
                    this.context);
 ...
 //               SSL.getMediaInstance(this.context, sslMediaChannel));
        ...
        modelChannelMap.put(reliableMulticastChannel.name, ReliableMulticast
                .getInstance(this.context, reliableMulticastChannel));
        ...
        if (this.networkingSwitch) {
            ...
            if (!this.isSSLSuppressed) {
                this.sslChannel.enable();
                this.sslMediaChannel.enable();
            }
        ...
        this.mReceiverRegistrar.registerReceiver(this.myNetworkReceiver,
                networkFilter);

        this.mListener = new PhoneStateListener() {
            public void onDataConnectionStateChanged(int state) {
                logger.trace(
                        "PhoneReceive::onCallStateChanged() - 3G status change {}",
                        state);
    ...
    public void onDestroy() {
        ...
        if (this.mReceiverRegistrar != null) {
            ...
            this.mReceiverRegistrar
                    .unregisterReceiver(this.mReadyResourceReceiver);
        }
        if (context != null) {
            context = null;
        }
    ...
    private String aggregatePref(final String key, final String def) {
        ...
        if (this.localSettings.contains(key) && (local != null)
                && local.equals(global)) {
            return local;
        }
        if (global == null) {
    ...
    private Boolean aggregatePref(final String key, final boolean def) {
        ...
        final Boolean global = Boolean.parseBoolean(this.globalSettings
                .getString(key, String.valueOf(local)));
    ...
    private Integer updatePref(final String key, final int def) {
        ...
        final Integer global = Integer.parseInt(this.globalSettings.getString(
                key, defStr));
        final Integer local = Integer.parseInt(this.localSettings.getString(
                key, defStr));
        if (this.localSettings.contains(key) && (local != null)
                && local.equals(global)) {
            return local;
        ...
        final boolean success = this.localSettings.edit()
                .putString(key, String.valueOf(global)).commit();
        if (!success) {
            logger.error("cannot update local setting {}", key);
        }
        return global;
    ...
    void acquirePreferences() {
        ...
        this.deviceId = this.localSettings.getString(
                INetPrefKeys.CORE_DEVICE_ID, this.deviceId);

        this.operatorId = this.aggregatePref(INetPrefKeys.CORE_OPERATOR_ID,
                this.operatorId);

        this.operatorKey = this.aggregatePref(INetPrefKeys.CORE_OPERATOR_KEY,
                this.operatorKey);

        PLogger.SET_PANTHR
                .debug("acquire device={} operator={} pass={}", new Object[] {
                        this.deviceId, this.operatorId, this.operatorKey });

        // JOURNAL
        this.isJournalUserDisabled = this.aggregatePref(
                INetPrefKeys.JOURNAL_DISABLED, this.isJournalUserDisabled);
        ...
        final String gatewayPortStr = this.localSettings.getString(
                INetPrefKeys.GATEWAY_PORT,
                String.valueOf(INetPrefKeys.DEFAULT_GATEWAY_PORT));
        int gatewayPort = Integer.valueOf(gatewayPortStr);

        final String flatLineTimeStr = this.localSettings.getString(
                INetPrefKeys.GATEWAY_FLAT_LINE_TIME,
        ...
        final String sslHostname = this.aggregatePref(
                INetPrefKeys.GATEWAY_HOST, INetPrefKeys.DEFAULT_GATEWAY_HOST);
         ...
         * Multicast
         */
        this.isMulticastSuppressed = this.aggregatePref(
                INetPrefKeys.MULTICAST_DISABLED,
                INetPrefKeys.DEFAULT_MULTICAST_ENABLED);

        final String multicastHost = this.localSettings.getString(
                INetPrefKeys.MULTICAST_HOST,
                INetPrefKeys.DEFAULT_MULTICAST_HOST);

        int multicastPort = Integer.parseInt(this.localSettings.getString(
                INetPrefKeys.MULTICAST_PORT,
                String.valueOf(INetPrefKeys.DEFAULT_MULTICAST_PORT)));

        long multicastFlatLine = Long.parseLong(this.localSettings.getString(
                INetPrefKeys.MULTICAST_NET_CONN_TIMEOUT,
                String.valueOf(INetPrefKeys.DEFAULT_MULTICAST_NET_CONN)));

        int multicastIdleTime = Integer.parseInt(this.localSettings.getString(
                INetPrefKeys.MULTICAST_CONN_IDLE_TIMEOUT,
                String.valueOf(INetPrefKeys.DEFAULT_MULTICAST_IDLE_TIME)));

        int multicastTTL = Integer.parseInt(this.localSettings.getString(
         ...
         * Reliable Multicast
         */
        this.isReliableMulticastSuppressed = this.aggregatePref(
                INetPrefKeys.RELIABLE_MULTICAST_DISABLED,
                INetPrefKeys.DEFAULT_RELIABLE_MULTICAST_ENABLED);

        final String reliableMulticastHost = this.localSettings.getString(
                INetPrefKeys.RELIABLE_MULTICAST_HOST,
                INetPrefKeys.DEFAULT_RELIABLE_MULTICAST_HOST);

        int reliableMulticastPort = Integer
                .parseInt(this.localSettings.getString(
                        INetPrefKeys.RELIABLE_MULTICAST_PORT,
                        String.valueOf(INetPrefKeys.DEFAULT_RELIABLE_MULTICAST_PORT)));

        long reliableMulticastFlatLine = Long
                .parseLong(this.localSettings
                        .getString(
                                INetPrefKeys.RELIABLE_MULTICAST_NET_CONN_TIMEOUT,
                                String.valueOf(INetPrefKeys.DEFAULT_RELIABLE_MULTICAST_NET_CONN)));

        int reliableMulticastIdleTime = Integer
                .parseInt(this.localSettings
                        .getString(
        ...
        int reliableMulticastTTL = Integer.parseInt(this.localSettings
                .getString(INetPrefKeys.RELIABLE_MULTICAST_TTL, String
                        .valueOf(INetPrefKeys.DEFAULT_RELIABLE_MULTICAST_TTL)));

        this.reliableMulticastChannel.setHost(reliableMulticastHost);
        this.reliableMulticastChannel.setPort(reliableMulticastPort);
        this.reliableMulticastChannel
                .setFlatLineTime(reliableMulticastFlatLine);
        this.reliableMulticastChannel
                .setSocketTimeout(reliableMulticastIdleTime);
         ...
         * SerialChannel
         */
        this.isSerialSuppressed = this.aggregatePref(
                INetPrefKeys.SERIAL_DISABLED,
                INetPrefKeys.DEFAULT_SERIAL_ENABLED);

        this.serialChannel
                .setDevice(this.localSettings.getString(
                        INetPrefKeys.SERIAL_DEVICE,
                        INetPrefKeys.DEFAULT_SERIAL_DEVICE));

        this.serialChannel
                .setBaudRate(Integer.parseInt(this.localSettings.getString(
                        INetPrefKeys.SERIAL_BAUD_RATE,
                        String.valueOf(INetPrefKeys.DEFAULT_SERIAL_BAUD_RATE))));

        this.serialChannel.setSlotNumber(Integer.parseInt(this.aggregatePref(
                INetPrefKeys.SERIAL_SLOT_NUMBER,
                String.valueOf(INetPrefKeys.DEFAULT_SERIAL_SLOT_NUMBER))));

        serialChannel.setRadiosInGroup(Integer.parseInt(this.aggregatePref(
                INetPrefKeys.SERIAL_RADIOS_IN_GROUP,
        ...
        serialChannel.setSlotDuration(Integer.parseInt(this.localSettings
                .getString(INetPrefKeys.SERIAL_SLOT_DURATION, String
                        .valueOf(INetPrefKeys.DEFAULT_SERIAL_SLOT_DURATION))));

        serialChannel
                .setTransmitDuration(Integer.parseInt(this.localSettings
                        .getString(
                                INetPrefKeys.SERIAL_TRANSMIT_DURATION,
                                String.valueOf(INetPrefKeys.DEFAULT_SERIAL_TRANSMIT_DURATION))));

        serialChannel.setSenderEnabled(this.localSettings.getBoolean(
                INetPrefKeys.SERIAL_SEND_ENABLED,
     ...
     * 
     * @param prefs
     *            a sharedPreferencesInterface for accessing and modifying
     *            preference data
     * @param key
     *            a string to signal which preference to access
     */
    final OnSharedPreferenceChangeListener pantherPreferenceChangeListener = new OnSharedPreferenceChangeListener() {
        ...
        public void onSharedPreferenceChanged(SharedPreferences prefs,
            ...
            if (key.equals(INetPrefKeys.CORE_DEVICE_ID)) {
                ...
                PLogger.SET_PANTHR.debug("device[{} -> {}]", parent.deviceId,
                        id);
            } else if (key.equals(INetPrefKeys.CORE_OPERATOR_ID)) {
                final String id = parent.updatePref(key, parent.operatorId);
                PLogger.SET_PANTHR.debug("operator[{} -> {}]",
                        parent.operatorId, id);
            } else if (key.equals(INetPrefKeys.CORE_OPERATOR_KEY)) {
                final String prev = (parent.operatorKey == null) ? INetPrefKeys.DEFAULT_CORE_OPERATOR_KEY
                        : parent.operatorKey;
            ...
            //
                ...
                PLogger.SET_PANTHR_GW.debug("suppress[{} -> {}]",
                        parent.isGatewaySuppressed, active);
                ...
                PLogger.SET_PANTHR_GW.debug("port[{} -> {}]",
                        INetPrefKeys.DEFAULT_GATEWAY_PORT, port);
                ...
                PLogger.SET_PANTHR_GW.debug("timeout[{} -> {}]",
                        INetPrefKeys.DEFAULT_GW_TIMEOUT, to);
            ...
            //
                ...
                final String prev = (parent.multicastChannel == null) ? INetPrefKeys.DEFAULT_MULTICAST_HOST
                        : parent.multicastChannel.getLocalIpAddresses().get(0)
                                .toString();
                final String host = parent.updatePref(key, prev);
                ...
                PLogger.SET_PANTHR_MC.debug("port[{} -> {}]",
                        INetPrefKeys.DEFAULT_MULTICAST_PORT, port);
                ...
                PLogger.SET_PANTHR_MC.debug("ttl[{} -> {}]",
                        INetPrefKeys.DEFAULT_MULTICAST_TTL, ttl);
            }
            ...
            //
                ...
                parent.updatePref(key,
                        INetPrefKeys.DEFAULT_RELIABLE_MULTICAST_HOST);
                final String prev = (parent.reliableMulticastChannel == null) ? INetPrefKeys.DEFAULT_RELIABLE_MULTICAST_HOST
                        : parent.reliableMulticastChannel.getLocalIpAddresses()
                                .get(0).toString();
                final String host = parent.updatePref(key, prev);
            ...
            //
                ...
                PLogger.SET_PANTHR_SERIAL.debug("device[{} -> {}]", prev,
                        device);
                ...
                PLogger.SET_PANTHR_SERIAL.debug("slot@[{} -> {}]", prev,
                        duration);
    ...
    final OnSharedPreferenceChangeListener ammoPreferenceChangeListener = new OnSharedPreferenceChangeListener() {
        final private NetworkManager parent = NetworkManager.this;
            ...
            try {
                ...
                if (key.equals(INetDerivedKeys.WIFI_PREF_IS_ACTIVE)) {
                    logger.trace("unprocessed key {}", key);
                        ...
                        .equals(INetDerivedKeys.PHYSICAL_LINK_PREF_IS_ACTIVE)) {
                    logger.trace("unprocessed key {}", key);
                    ...
                    final String prev = (parent.operatorId == null) ? INetPrefKeys.DEFAULT_CORE_OPERATOR_ID
                            : parent.operatorId;
                    final String id = parent.operatorId = prefs.getString(key,
                            prev);
                    ...
                    final String prev = (parent.operatorKey == null) ? INetPrefKeys.DEFAULT_CORE_OPERATOR_KEY
                            : parent.operatorKey;
                    final String pass = parent.operatorKey = prefs.getString(
                            key, prev);
                    PLogger.SET_PANTHR.debug("pass[{} -> {}]", prev, pass);
                    ...
                    final Integer timeout = Integer.valueOf(prefs.getString(
                            key,
                            String.valueOf(INetPrefKeys.DEFAULT_GW_TIMEOUT)));
                    parent.tcpChannel
                            .setSocketTimeout(timeout.intValue() * 1000);
                    ...
                    String ipAddress = prefs.getString(key,
                            INetPrefKeys.DEFAULT_MULTICAST_HOST);
                ...
                //
                    ...
                    serialChannel
                            .setBaudRate(Integer.parseInt(prefs.getString(
                                    key,
                                    String.valueOf(INetPrefKeys.DEFAULT_SERIAL_BAUD_RATE))));
                } else if (key.equals(INetPrefKeys.SERIAL_SLOT_NUMBER)) {
                    serialChannel
                            .setSlotNumber(Integer.parseInt(prefs.getString(
                                    key,
                                    String.valueOf(INetPrefKeys.DEFAULT_SERIAL_SLOT_NUMBER))));
                } else if (key.equals(INetPrefKeys.SERIAL_RADIOS_IN_GROUP)) {
                    serialChannel
                            .setRadiosInGroup(Integer.parseInt(prefs.getString(
                                    key,
                                    String.valueOf(INetPrefKeys.DEFAULT_SERIAL_RADIOS_IN_GROUP))));
                } else if (key.equals(INetPrefKeys.SERIAL_SLOT_DURATION)) {
                    serialChannel
                            .setSlotDuration(Integer.parseInt(prefs.getString(
                                    key,
    ...
    public AmmoMessages.MessageWrapper.Builder buildAuthenticationRequest() {
        ...
        final AmmoMessages.AuthenticationMessage.Builder authreq = AmmoMessages.AuthenticationMessage
                .newBuilder();
        authreq.setDeviceId(UniqueIdentifiers.device(this.context))
    ...
    public DisposalState sendRequest(AmmoGatewayMessage agm, String channelName) {
        logger.info(
                "Ammo sending Request size ({}) priority({}) to Channel {}",
    ...
    public boolean isConnected() {
        boolean any = (tcpChannel.isConnected()
                ...
                || reliableMulticastChannel.isConnected() || ((serialChannel != null) && serialChannel
                .isConnected()));
     ...
     */
    public void authorizationSucceeded(NetChannel channel,
        ...
        logger.trace(
                "authentication complete, repost subscriptions and pending data {}",
                channel);
    ...
    public void statusChange(NetChannel channel, int lastConnStatus,
        ...
        if (modelChannel == null) {
            ...
            logger.trace("Here is the 'modelChannelMap' => {}", modelChannelMap);
        } else {
            modelChannel.setStatus(new int[] { connStatus, sendStatus,
                    recvStatus });
        }
        ...
        if (lastConnStatus != connStatus) {
            ...
            broadcastIntent.putExtra(AmmoIntents.EXTRA_CONNECT_STATUS,
                    connStatus);
    ...
    // Network Channels
    final private TcpChannel tcpChannel = TcpChannel.getInstance(
            ChannelFilter.GATEWAY, this);
    final private MulticastChannel multicastChannel = MulticastChannel
            .getInstance(ChannelFilter.MULTICAST, this);
    final private ReliableMulticastChannel reliableMulticastChannel = ReliableMulticastChannel
            .getInstance(ChannelFilter.RELIABLE_MULTICAST, this, this.context);
    final private JournalChannel journalChannel = JournalChannel.getInstance(
            ChannelFilter.JOURNAL, this);
    private SerialChannel serialChannel = null;

    final private TcpChannel tcpMediaChannel = TcpChannel.getInstance(
            ChannelFilter.GATEWAYMEDIA, this);

    // added for SSL
    final private SSLChannel sslChannel = SSLChannel.getInstance(
            ChannelFilter.SSL, this);
    final private SSLChannel sslMediaChannel = SSLChannel.getInstance(
            ChannelFilter.SSLMEDIA, this);

    final public List<NetChannel> registeredChannels = new ArrayList<NetChannel>();
    ...
    private class NetworkBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(final Context context, final Intent aIntent) {
            ...
            if (AmmoIntents.AMMO_ACTION_ETHER_LINK_CHANGE.equals(action)) {
                ...
                // if this is not for wifi it has to be wired .. bad logic maybe
                ...
                logger.error("Serial link changed. devname: {}, state: {}",
                        devname, state);
